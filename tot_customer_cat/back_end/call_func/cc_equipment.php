<?php
	...
	if(isset($_POST['load_cc_equipment_detail'])){
		...
		if(isset($_POST['filter']) && $_POST['filter']!=""){
			$option="where cc_type.t_id=cc_customer.t_id && cc_equipment.c_id=cc_customer.c_id && cc_customer.pv_id='{$_SESSION['pv_id']}' && cc_equipment.p_id=cc_product.p_id && cc_customer.c_id = '{$_POST['filter']}' ";
		}else{
			$option="where cc_type.t_id=cc_customer.t_id && cc_equipment.c_id=cc_customer.c_id && cc_customer.pv_id='{$_SESSION['pv_id']}' && cc_equipment.p_id=cc_product.p_id  limit {$start},{$perpage}";
		}

		echo $db->select("cc_equipment,cc_customer,cc_product,cc_type","*",$option);
		...
		$data_convert="eq_des='{$_POST['eq_des']}',eq_amount='{$_POST['eq_amount']}',eq_code='{$_POST['eq_code']}',eq_price='{$_POST['eq_price']}',eq_discount='{$_POST['eq_discount']}'";
